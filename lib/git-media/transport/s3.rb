    ...
    class S3 < Base
      ...
      def get_media_files
        ...
        begin
          ...
          # have been listed
          @s3.incrementally_list_bucket(@bucket) do |contents|
            contents[:contents].each do |element|
              files << { :size element[:size], path: @bucket, name: element[:key], sha: element[:key] }
            end
          end
        rescue
          []
